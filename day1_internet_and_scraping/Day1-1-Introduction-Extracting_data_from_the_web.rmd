...
We will learn more about APIs on Wednesday and Thursday.
...
We will work with such data on Thursday.

#### Accessing DBs directly

Sometimes you can gain direct access to certain databases. Then you can connect to the database over a special connection and query the database directly.
...
`main > h1` selects all the h1-tags (big headlines) that are children of "main"-tag.
Then the stuff between the curly braces defines the font, the size and look of the headline.
...
**Exercise 1.3 (hands-on):** Scrape the headlines from a popular news-website of your choosing!
...
result_folder <- "D:/dev/r/EDSD_2022_working_with_digital_data/day1-Internet/data/"
...
# Create a dataframe from all the data (note that we didn't really need the two blocks before):
dflinks <- data.frame(
 ...
 * For other website owners, it is not okay - but you are probably still allowed to scrape it!
   * A US court ruled, that LinkedIn has to remove technical measures that prevented a startup from scraping public profile information of LinkedIn users [source](https://www.reuters.com/article/us-microsoft-linkedin-ruling-idUSKCN1AU2BV?il=0)
   ...
   * If you have an account: it may get blocked or suspended. Don't use accounts that are important to you for web-scraping!
...
## Code-session: Build a news-scraper
